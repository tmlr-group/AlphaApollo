...
class InformalMathTrainingMultiProcessEnv(gym.Env):
    """
    ...
    def __init__(
        ...
        from agent_system.environments.informal_math_training.env import InformalMathTrainingEnv
        ...
        # ---------- Key changes start ----------
        informal_math_cfg  = env_config.informal_math_training

        # 2) Assign configuration to each env in a round-robin manner
        self.envs = []
        for idx in range(self.batch_size):
            cfg_i = deepcopy(informal_math_cfg)
            self.envs.append(InformalMathTrainingEnv(cfg_i))
    ...
    def _sync_reset(self, env, kwargs):
        ...
        env.reset(extras)
        obs = extras["question"] # the initial state should be the reference molecule
        info = {
            "data_source": kwargs.get("data_source", "unknown")
        }
        return obs, info
...
def build_informal_math_training_envs(
    seed: int = 0,
    env_num: int = 1,
    group_n: int = 1,
    is_train: bool = True,
    ...
    return InformalMathTrainingMultiProcessEnv(
        seed=seed,
        env_num=env_num,
        group_n=group_n,
        is_train=is_train,
